...
if __name__=='__main__':
    ...
    photo = photo.requires_grad_().cpu()  # .cpu()  
    ...
    result_show = normalize(result)
    plt.imshow(result_show)
    # plt.show()


    result4PIL = Image.fromarray(np.uint8(result_show*255))
    ...
    # draw = ImageDraw.Draw(result4PIL) 
    # draw.rectangle(
    #     [(w-nw)//2, (h-nh)//2, nw+(w-nw)//2, nh+(h-nh)//2],
    #     outline=(255,0,0),
    #     width=8,
    # )
    
    box = [(w-nw)//2, (h-nh)//2, nw+(w-nw)//2, nh+(h-nh)//2]
    result4PIL = result4PIL.resize((iw,ih),box=box)
    result4PIL.show()
