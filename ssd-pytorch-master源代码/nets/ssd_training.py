...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        # 转化成Variable
        # 不清楚这两行代码作用,注释掉仍然能运行
        loc_t = Variable(loc_t, requires_grad=False)  # torch.Size([4, 8732, 4])  
        ...
        # 计算回归loss
        pos_idx = pos.unsqueeze(pos.dim()).expand_as(loc_data)  # torch.Size([4, 8732, 4])
        loc_p = loc_data[pos_idx].view(-1, 4)  # torch.Size([44, 4])  # 预测框的位置信息
        loc_t = loc_t[pos_idx].view(-1, 4)  # torch.Size([44, 4])     # 真值框的位置信息
        # loss_l = F.smooth_l1_loss(loc_p, loc_t, size_average=False)  
        loss_l = F.smooth_l1_loss(loc_p, loc_t, reduction='sum')  # 建议使用这行代码,上一行已废弃

        # 转化形式
        batch_conf = conf_data.view(-1, self.num_classes)  # torch.Size([34928, 3])
        # 你可以把softmax函数看成一种接受任何数字并转换为概率分布的非线性方法
        # 获得每个框预测到真实框的类的概率
        loss_c = log_sum_exp(batch_conf) - batch_conf.gather(1, conf_t.view(-1, 1))  # torch.Size([34928, 1])
        ...
        # 限制负样本数量
        num_neg = torch.clamp(self.negpos_ratio*num_pos, max=pos.size(1)-1)  # torch.Size([4, 1])
        neg = idx_rank < num_neg.expand_as(idx_rank)  # torch.Size([4, 8732])
        # 筛选出前self.negpos_ratio*num_pos个样本

        # 计算正样本的loss和负样本的loss
        pos_idx = pos.unsqueeze(2).expand_as(conf_data)  # torch.Size([4, 8732, 3])
        neg_idx = neg.unsqueeze(2).expand_as(conf_data)  # torch.Size([4, 8732, 3])
        conf_p = conf_data[(pos_idx+neg_idx).gt(0)].view(-1, self.num_classes)  # torch.Size([176, 3])
        ...
        loss_c = F.cross_entropy(conf_p, targets_weighted, reduction='sum')  # 建议使用这行代码,上一行已废弃
        # torch.Size([176, 3])
        # torch.Size([176])
